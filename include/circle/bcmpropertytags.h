 ...
 * @brief リクエスト属性を持たない単純な属性タグを表す構造体
 */
struct TPropertyTagSimple
{
    TPropertyTag    Tag;
 ...
 * @brief EDIDブロックの取得を行う属性タグを表す構造体（HDMI/DVI機器）
 */
struct TPropertyTagEDIDBlock
{
    TPropertyTag    Tag;
...
struct TPropertyTagAllocateBuffer
{
    ...
    union
    {
        u32         nAlignment;         // バイト単位（リクエスト）
        u32         nBufferBaseAddress; // （レスポンス）
    }
    PACKED;
    u32             nBufferSize;
}
 ...
 * @brief ディスプレイの幅/高さの設定を行う属性タグを表す構造体
 */
struct TPropertyTagDisplayDimensions
{
    TPropertyTag    Tag;
 ...
 * @brief 仮想（バッファ）の幅/高さの設定を行う属性タグを表す構造体
 */
struct TPropertyTagVirtualOffset
{
    TPropertyTag    Tag;
...
struct TPropertyTagSetPalette
{
    ...
    union
    {
        u32         nOffset;         // セットする最初のパレットインデックス（0-255）
        u32         nResult;
    #define SET_PALETTE_VALID   0
    }
    PACKED;
    u32             nLength;         // 設定するパレットエントリの数（1-256）
    u32             Palette[0];      // RGBAパレット値（オフセット - オフセット+長さ-1）
}
PACKED;

/**
 * @struct TPropertyTagCommandLine
 * @brief コマンドラインの取得を行う属性タグを表す構造体
 */
struct TPropertyTagCommandLine
{
    TPropertyTag    Tag;
...
{
public:
    CBcmPropertyTags (boolean bEarlyUse = FALSE);   // 初期にスピンロックを使用しない
    ~CBcmPropertyTags (void);

    boolean GetTag (u32 nTagId,                     //< タグID
            void       *pTag,                       //< タグ構造体へのポインタ
            unsigned    nTagSize,                   //< タグ構造体のサイズ
            unsigned    nRequestParmSize = 0);      //< パラメタバイト数

    boolean GetTags (void  *pTags,                  //< タグ構造体へのポインタ
             unsigned       nTagsSize);             //< タグ構造体のサイズ
