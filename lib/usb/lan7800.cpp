...
boolean CLAN7800Device::Configure (void)
{
    ...
    while ((pEndpointDesc = (TUSBEndpointDescriptor *) GetDescriptor (DESCRIPTOR_ENDPOINT)) != 0)
    {
        ...
        {
            if ((pEndpointDesc->bEndpointAddress & 0x80) == 0x80)   // 入力
            {
    ...
    if (   !WriteReg (BURST_CAP, DEFAULT_BURST_CAP_SIZE / HS_USB_PKT_SIZE)    // USB HS用
...
TNetDeviceSpeed CLAN7800Device::GetLinkSpeed (void)
{
    ...
    // 自動ネゴシエーションされたかチェック
    assert (!(usPHYAuxControlStatus & (1 << 14)));        // 自動ネゴシエーションは有効か?
