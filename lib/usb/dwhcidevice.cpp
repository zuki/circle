...
boolean CDWHCIDevice::SubmitBlockingRequest (CUSBRequest *pURB, unsigned nTimeoutMs)
{
    ...
    assert (pURB != 0);     // リクエストはセットされている

    pURB->SetStatus (0);    // ステータスコードのデフォルト値（成功）を設定
    ...
    if (pURB->GetEndpoint ()->GetType () == EndpointTypeControl)
    {
        ...
        if (pSetup->bmRequestType & REQUEST_IN)     // INリクエスト
        {
            assert (pURB->GetBufLen () > 0);
            //                           IN?    statusStage?
            if (   !TransferStage (pURB, FALSE, FALSE)  // out command
                || !TransferStage (pURB, TRUE,  FALSE)  // in data
                || !TransferStage (pURB, FALSE, TRUE))  // out ack
            {
                return FALSE;
            }
        }
        else    // IN以外のリクエスト
        {
            ...
            {
                if (   !TransferStage (pURB, FALSE, FALSE)  // out command
                    || !TransferStage (pURB, TRUE,  TRUE))  // in ack
                {
                    return FALSE;
                }
            }
            ...
            {
                if (   !TransferStage (pURB, FALSE, FALSE)  // out commdand
                    || !TransferStage (pURB, FALSE, FALSE)  // out data
                    || !TransferStage (pURB, TRUE,  TRUE))  // in ack
                {
                    return FALSE;
                }
    ...
    else    // バルク転送かインタラプト転送（アイソクロナス転送は未サポート）
    {
...
boolean CDWHCIDevice::EnableRootPort (void)
{
    ...
    if (pOptions != 0)
    {
        // 電源オン遅延時間が指定されていたらそれをnMsDelayにセット
    ...
    m_pTimer->MsDelay (100);    // see USB 2.0 spec: Table 7-13
    // ホストポートをリセット
    HostPort.Read ();
    HostPort.And (~DWHCI_HOST_PORT_DEFAULT_MASK);
    ...
    m_pTimer->MsDelay (50);        // see USB 2.0 spec: 7.1.7.5 (tDRSTR)
    // リセットリカバリ
    HostPort.Read ();
    HostPort.And (~DWHCI_HOST_PORT_DEFAULT_MASK);
...
#else
    unsigned nChannel = DWHCI_MAX_CHANNELS;        // 使用しない
#endif
                  ...
                  unsigned      nMsTimeout)
{
    ...
    {
        assert (m_pTimer != 0);
        m_pTimer->MsDelay (1);      // 1ミリ秒待つ

        if (--nMsTimeout == 0)      // 指定タイムアウトを過ぎたら
        {                           // falseを返す
            //LOGWARN ("Timeout");
#ifndef NDEBUG
            //pRegister->Dump ();
