...
int CSocket::Bind (u16 nOwnPort)
{
    ...
    m_nOwnPort = nOwnPort;                  // TODO: ポートの重複利用を抑制する

    if (m_nProtocol == IPPROTO_UDP)         // UDPの場合はUDPバインド処理
    {
        m_hConnection = m_pTransportLayer->Bind (m_nOwnPort, m_nProtocol);
        if (m_hConnection < 0)
        {
            return m_hConnection;           // エラーコードを返す
        }
    }
...
int CSocket::Connect (CIPAddress &rForeignIP, u16 nForeignPort)
{
    ...
    assert (m_pNetConfig != 0);
    if (   m_pNetConfig->GetIPAddress ()->IsNull ()     // ヌルソースアドレスから
        && !(   m_nProtocol == IPPROTO_UDP              // UDPブロードキャスだけが許可されている
...
CSocket *CSocket::Accept (CIPAddress *pForeignIP, u16 *pForeignPort)
{
    ...
    // コネクションを探す。すでにアクティブか、そうでなければまずアクティブにする（最小のハンドルを持つ）
#define INT_MAX        0x7FFFFFFF
    ...
    // 返されたコネクションを新しくリッスンしたコネクションに置き換える
...
int CSocket::Receive (void *pBuffer, unsigned nLength, int nFlags)
{
    ...
    if (nLength < (unsigned) nResult)   // 指定されたバッファ長より結果のほうが長い場合は切り詰める
    {
        nResult = nLength;
    }
