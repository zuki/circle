...
#define TCP_MAX_CONNECTIONS 1000    // 最大アクティブTCPコネクション数

#define MSS_R               1480    // ネットワーク層から受信する最大セグメントサイズ
#define MSS_S               1480    // ネットワーク層に送信する最大セグメントサイズ

#define TCP_CONFIG_MSS      (MSS_R - 20)
#define TCP_CONFIG_WINDOW   (TCP_CONFIG_MSS * 10)

#define TCP_CONFIG_RETRANS_BUFFER_SIZE  0x10000 // 最大送信ウインドウサイズより大きくなければならない

#define TCP_MAX_WINDOW      ((u16) -1)  // ウィンドウ拡張オプションはなし
#define TCP_QUIET_TIME      30      // クラッシュ後に次のコネクションを可視するまでの秒数

#define HZ_TIMEWAIT         (60 * HZ)
#define HZ_FIN_TIMEOUT      (60 * HZ)   // FIN-WAIT-2状態のタイムアウト
...
struct TTCPHeader
{
    ...
    /// @brief データオフセット
    u16     nDataOffsetFlags;   // 以下の#define(s)はBE()なしでも正しい
...
#define TCP_HEADER_SIZE     20      // 通常のデータセグメントではTCPオプションなしでもvalid

/// @brief TCPオプションをあらわす構造体
struct TTCPOption
{
    /// @brief 種別
    u8  nKind;              // Data:
#define TCP_OPTION_END_OF_LIST  0    // なし（長さフィールドなし）
#define TCP_OPTION_NOP          1    // なし（長さフィールドなし）
#define TCP_OPTION_MSS          2    // 最大セグメントサイズ（2バイト）
#define TCP_OPTION_WINDOW_SCALE 3    // シフトカウント（1バイト）
#define TCP_OPTION_SACK_PERM    4    // なし
#define TCP_OPTION_TIMESTAMP    8    // タイムスタンプ値、タイムスタンプエコーリレー（2*4バイト）
    /// @brief 長さ
    u8  nLength;
    /// @brief オプションデータ
    u8  Data[];
